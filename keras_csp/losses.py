...
# categorical_crossentropy is not used in this file, removed for compatibility
# from keras.objectives import categorical_crossentropy

# For compatibility with Keras 2.x
try:
    if K.image_dim_ordering() == 'tf':
        import tensorflow as tf
except AttributeError:
    # Keras 2.x uses image_data_format instead, but we always need tf
    import tensorflow as tf
...
def cls_center(y_true, y_pred):
	...
	class_loss = 0.1*tf.reduce_sum(focal_weight*classification_loss) / tf.maximum(1.0, assigned_boxes)
...
def cls_center_full(y_true, y_pred):
	...
	negatives = y_true[:, :, :, 1]-y_true[:, :, :, 2]
	foreground_weight = positives * (1.0 - y_pred[:, :, :, 0]) ** 2.0
	background_weight = negatives * ((1.0 - y_true[:, :, :, 0])**4.0)*(y_pred[:, :, :, 0] ** 2.0) / 16.0
...
# Crowdedness-aware regression loss
def regr_h(y_true, y_pred):
	...
	l1_loss = y_true[:, :, :, 2] * y_true[:, :, :, 1]*tf.where(tf.less(absolute_loss, 1.0), square_loss, absolute_loss - 0.5)
...
# 	loss = y_true[:, :, :, 3] * y_true[:, :, :, 2] * tf.reduce_sum(tf.where(tf.less(absolute_loss, 1.0), square_loss, absolute_loss - 0.5),axis=-1)
# 	assigned_boxes = tf.reduce_sum(y_true[:, :, :, 2])

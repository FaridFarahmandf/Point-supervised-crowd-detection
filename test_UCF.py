...
C.num_scale = 1
C.down = 1
cache_path = 'data/cache/UCF/test'
...
if C.offset:
    w_path = 'output/valmodels/UCF/%s/off' % (C.scale)
    out_path = 'output/valresults/UCF/%s/off' % (C.scale)
else:
    w_path = 'output/valmodels/UCF/%s/nooff' % (C.scale)
    out_path = 'output/valresults/UCF/%s/nooff' % (C.scale)
...
for f in range(num_imgs):
    ...
    keep_index = np.where(np.minimum(det[:, 2] - det[:, 0], det[:, 3] - det[:, 1]) >= 2)[0]
    dets = det[keep_index, :]

...
C.onegpu = 4
C.size_train = (256, 256)
C.init_lr = 2e-4 * (num_gpu/8.)
C.offset = False
C.scale = 'h'
C.num_scale = 1
C.num_epochs = 30
C.iter_per_epoch = 100
C.down = 1
batchsize = C.onegpu * num_gpu
os.environ["CUDA_VISIBLE_DEVICES"] = C.gpu_ids

if C.offset:
    C.path_history = None#''output/valmodels/UCF/%s/off' % (C.scale)
else:
    C.path_history = None#'output/valmodels/UCF/%s/nooff' % (C.scale)

C.update_thr = 0.6
C.update_weights_thr = 10
C.alpha = 1
# get the training data
cache_path = 'data/cache/UCF/train'
cache_path_updated = 'data/cache/UCF/train_updated'
...
# Note: ParallelModel may not work with TensorFlow 2.x
# If you encounter errors, set num_gpu to 1 or use TensorFlow 1.x
if num_gpu > 1:
    try:
        from keras_csp.parallel_model import ParallelModel
        model = ParallelModel(model, int(num_gpu))
    except Exception as e:
        print('Warning: Could not use ParallelModel, using single GPU. Error: {}'.format(e))
...
if C.path_history is not None:
    ...
    path_model = C.path_history + '/net_last.hdf5'
    ...
    if not os.path.exists(weight_path):
        ...
        print('=' * 80)
        print('Please download ResNet-50 weights from:')
        print('https://github.com/fchollet/deep-learning-models/releases/download/v0.2/resnet50_weights_tf_dim_ordering_tf_kernels.h5')
        print('')
        print('Or use this command to download:')
        print('wget https://github.com/fchollet/deep-learning-models/releases/download/v0.2/resnet50_weights_tf_dim_ordering_tf_kernels.h5')
        print('')
        print('Then move it to: {}'.format(weight_path))
        print('=' * 80)
        raise FileNotFoundError('Weight file not found at: {}'.format(weight_path))
    model.load_weights(weight_path, by_name=True)
    print('load weights from {}'.format(weight_path))
    print("Starting from scratch...")

    if C.offset:
        out_path = 'output/valmodels/UCF/%s/off' % (C.scale)
    else:
        out_path = 'output/valmodels/UCF/%s/nooff' % (C.scale)
...
### for validation - using test set as validation
cache_path_val = 'data/cache/UCF/test'
...
# Use ShanghaiTechA data generator class (same structure)
...
try:
    ...
    # For newer Keras versions, this might not be needed
    pass
        ...
        try:
            ...
            if iter_num == epoch_length:
                ...
                if cls_loss1 < best_loss:
                    ...
                    history["path_best_loss_model"] = os.path.join(out_path,'net_e{}_l{}.hdf5'.format(epoch_num + 1 + add_epoch,cls_loss1))
            ...
            continue
    if epoch_num+1>30  and epoch_num % 1 == 0:
        ...
        if mae <= best_mae:
            ...
            history["path_best_model"] = os.path.join(out_path,'net_e{}.hdf5'.format(epoch_num + 1 + add_epoch))
            if epoch_num + 1 > 30:
                model.save_weights(os.path.join(out_path, 'net_e{}.hdf5'.format(epoch_num + 1 + add_epoch)))
            save_json(C.path_history + '/history.json', history)
